...
- typing_checkpoint: chunkごとの最新カーソル、入力数、ミス数、累積経過時間、開始時刻、
  自動インデント設定。TypingまたはPause中に最大1秒間隔でupsertする。
...
6. ResultのEnterは次のおすすめを開始し、`r`は同じファイルを再挑戦する。
   Esc / `t` でTreeへ戻ると更新済み進捗が表示される。

### 中断と再開

TypingのEscはポーズを開きます。ポーズの`t`またはCtrl-Cでは現在のチェックポイントを
即時保存し、ファイル完了を更新せずTreeへ戻ります。次回に同じファイルを開くと、カーソル、
入力数、ミス数、累積経過時間を復元します。Pauseの「最初からやり直し」だけは未完了セッションを
確定してチェックポイントを削除します。Resultは完走時だけ表示します。
